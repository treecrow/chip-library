...
"use strict";
//content-env 与  extension-env 通信
...
var userCenter = __webpack_require__(4);

setInterval(function () {
  chrome.storage.local.get('lastCheckTime', function (data) {
    if (!data) {
      data = 0;
    }
    if (window.forbidFavor) {
      return;
    }
    userCenter.checkFavor(data).then(function (res) {
      res = JSON.parse(res);
      console.log(res);
      if (res.data.num) {
        chrome.browserAction.setBadgeText({
          text: res.data.num.toString()
        });
      }
    });
  });
...
var userData = __webpack_require__(12);
var communicate = __webpack_require__(2);
        ...
        module.exports.get(G.server + '/extension/CouponHandler?op=get&rawApi=' + encodeURIComponent(url)).then(function (res) {
            if (res.map && res.map.length > 0 && res.map[0].is_api && res.handle) {
                var _ret = (function () {
                    ...
                    var NamedRe = __webpack_require__(7).named;
